...
void Stats::populateTable(QTableWidget* table)
{
	...
	table->setHorizontalHeaderLabels({ "Enveloppe", "Par mois", "Par année" });
	...
	for(const auto& env : g_enveloppeManager.getEnveloppes())
	{
		...
		if(monthCount >= 12)
		{
			...
			perYearStr = QString::number(perYearAvg);
		}
		else
			perYearStr = "n/a";
		...
		table->insertRow(row);
		table->setItem(row, 0, new QTableWidgetItem(QString::fromStdString(env.getName())));
		table->setItem(row, 1, new QTableWidgetItem(QString::number(perMonthAvg)));
		table->setItem(row, 2, new QTableWidgetItem(perYearStr));
	}
}
...
QHBoxLayout* Stats::createDateDropdowns() //refactor
{
